...
### hclust()
wine_dist <- dist(wine_df)

wine_dist_man <- dist(wine_df, method = "manhattan")

factoextra::fviz_dist(wine_dist)

factoextra::fviz_dist(wine_dist_man)
...
### to cut the tree use the cutree()
cutree(wineH4, k = 3)

cutree(wineH4, h = 25)

### count the number observations in each cluster
wine %>% 
...
wine %>% 
  mutate(cluster_id = cutree(wineH4, k = 3)) %>% 
  ggplot(mapping = aes(x = as.factor(cluster_id))) +
  geom_bar() +
  theme_bw()
